...
class Core_Lib_Seccode
{
    ...
    var $fontcolor;
    var $im;
    var $charset = 'utf-8';   //默认字符集, 可选 'gbk', 'big5', 'utf-8'
    ...
    function image ()
    {
        ...
        if ($this->animator == 1 && function_exists ('imagegif')) {
            ...
            $anim = new Core_Lib_GifMerge ($frame, 255, 255, 255, 0, $d, $x, $y, 'C_MEMORY');
            $this->dheader ('Content-type: image/gif');
            ...
            if (function_exists ('imagepng')) {
                $this->dheader ('Content-type: image/png');
                imagepng ($this->im);
            } else {
                $this->dheader ('Content-type: image/jpeg');
...
//            }
    ...
    function ttffont ()
    {
        ...
        if ($this->type && !empty ($seccodettf)) {
            if (strtoupper ($charset) != 'UTF-8') {
                $cvt = new Core_Lib_Chinese ($charset, 'utf8');
    ...
    function bitmap ()
    {
        ...
        $this->dheader ('Content-Type: image/bmp');
        echo $image;
    }
}
