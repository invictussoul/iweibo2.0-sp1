...
class Core_Comm_Util
{
    ...
    static public function array2string ($array, $sep=",", $out="'",$escape=true)
    {
        if (!empty ($array)) {
            ...
            foreach ($array as $value)
            {
                if($escape)
                {
                    $r .= $out . Core_Db::sqlescape($value) . $out . $sep;
                }else{
                    $r .= $out . $value . $out . $sep;
                }
            }
    ...
    {
        foreach(Core_Comm_City::$cityConfig as $key => $value)
            $list[$key] = $value['name'];
        return $list;
    }
     ...
     * @return array
     */
    public static function getProvinceList($nationIndex)
    {
        foreach(Core_Comm_City::$cityConfig[$nationIndex]['province'] as $key => $value)
            $list[$key] = $value['name'];
        return $list;
    }
     ...
     * @return array
     */
    public static function getCityList($nationIndex, $provinceIndex)
    {
        foreach(Core_Comm_City::$cityConfig[$nationIndex]['province'][$provinceIndex]['city'] as $key => $value)
            $list[$key] = $value;
        return $list;
    }
     ...
     * @return array
     */
    public static function getLocation($nationIndex, $provinceIndex, $cityIndex)
    {
        $location = '';
        if($nationIndex>1 && isset(Core_Comm_City::$cityConfig[$nationIndex]['name'])) $location = Core_Comm_City::$cityConfig[$nationIndex]['name'].' ';
        isset(Core_Comm_City::$cityConfig[$nationIndex]['province'][$provinceIndex]['name']) && $location .= Core_Comm_City::$cityConfig[$nationIndex]['province'][$provinceIndex]['name'];
        isset(Core_Comm_City::$cityConfig[$nationIndex]['province'][$provinceIndex]['city'][$cityIndex]) && $location .= ' '.Core_Comm_City::$cityConfig[$nationIndex]['province'][$provinceIndex]['city'][$cityIndex];
     ...
     * @author echoyang
     */
    function simcn2tracn($data,$type=1, $charset = 'utf-8')
    {
            ...
            if(strtolower($charset) == 'utf-8') $word = 3;
            elseif(strtolower($charset) == 'gb2312') $word = 2;
            ...
            for($i = 0; $i <= strlen($data); $i++) {
                    if($i != 0 && $i % $word == 0) {
                            ...
                            if($i != strlen($data)) $cnword = $data{$i};
                    } else {
                            $cnword .= $data{$i};
                    }
            }
    ...
    public static function getUserCachePath($uname,$relCahceDir)
    {
        if(empty($uname)) throw new Core_Exception('没有用户错误');
        if(empty($relCahceDir)) throw new Core_Exception('没有缓存地址');
        $userCacheFile = $userDir = '';

        $md5NameStr = md5($uname);
        $tDirMd5 = substr($md5NameStr, -2); //加name的md5值倒数第1~2位做1级文件夹
        $userDir = $relCahceDir . $tDirMd5 . '/';

        $tDirMd5 = substr($md5NameStr, -4, 2); //加name的md5值倒数第3~4位做2级文件夹
        $userDir .= $tDirMd5 . '/';

        $tDirMd5 = substr($md5NameStr, -6, 2); //加name的md5值倒数第5~6位做3级文件夹
        $userDir .= $tDirMd5 . '/';

        $userCacheFile = $userDir . $uname . '.php';  //我收听好友缓存文件
    ...
    public static function formatName2Array($names)
    {
        if(empty($names) && isset($_SESSION['name']))
        {
            return array($_SESSION['name']);//为空是获取自己消息
        }
