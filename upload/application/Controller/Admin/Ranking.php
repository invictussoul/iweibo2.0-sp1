 ...
 */
class Controller_Admin_Ranking extends Core_Controller_Action
{
    //大家都在说Model
    private $_rankingModel = null;
    ...
    public function __construct($params)
    {
        ...
        $this->_rankingModel = new Model_Ranking();    //实例化『大家都在说』Model.
    ...
    public function indexAction()
    {
        ...
        if ('post' == $action)    //处理提单修改的表单请求
        {
            ...
            if (isset($config['component.ranking']))
            {
                $accountList = $config['component.ranking'];
                ...
                if (!$validatedAccount)
                {
                    ...
                    $this->showmsg('设置列表出错，请检查您的设置是否正确。', '/admin/ranking/index/!');
                }

                //保存大家都在说设置.
                $this->_rankingModel->saveRanking($validatedAccount);
                $this->setCookie('errorMessage', false);
                $this->setCookie('accounts', false);
                $this->showmsg('您的列表设置成功，正在返回..', '/admin/ranking/index');
            }
        }
        else
        {
    ...
    private function _validateAccount($accountList = array())
    {
        ...
        {
            $this->_errorMessage[] = '您设置的排行榜信息不能为空';
            return false;
        }
