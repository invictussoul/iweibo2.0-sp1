...
$(function () {
    $(".followunfollowmore").click(function (event) {
        var self = $(this);
        var forkey = self.attr("data-forkey");
        var pplname = [];
        var ppls = $("a[data-key=" + forkey + "]");
        var func = [event.ctrlKey ? "un":"","follow"].join("");
        var posConfig = {
            0: {
                 0:["-52px", "-27px"]
                ,1:["-52px", "0px"]
        ...
        $.each(ppls,function (i ,ppl) {
            pplname.push($(ppl).attr("data-name"));
        });
        
        IWB_API[func]("quickfoufo" ,pplname.join(",") ,function (identity ,response) {
            if (identity === "quickfoufo" && response.ret === 0) {
                $.each(ppls, function (i, ppl) { // 全部收听成功改变按钮为取消收听
                    var ppl = $(ppl);
                    var styleid = ppl.attr("data-styleid");
                    if (func === "follow" && ppl.attr("data-type") === "1") { // 收听按钮，已经是取消收听忽略
                        ppl.attr("data-type",0);
                        ppl.attr("title","取消收听");
                        ppl.css({
                            "backgroundPosition" : posConfig[styleid][ppl.attr("data-type")].join(" ")
                        });
                    } else if (func === "unfollow" && ppl.attr("data-type") === "0") { // 取消收听按钮，已经是收听的忽略
                        ppl.attr("data-type",1);
                        ppl.attr("title","收听");
                        ppl.css({
                            "backgroundPosition" : posConfig[styleid][ppl.attr("data-type")].join(" ")
                        });
                ...
                IWB_DIALOG.modaltipbox("success" ,(func === "unfollow" ? "取消":"") + "收听他/她们成功");
            }
        });
    });
});
