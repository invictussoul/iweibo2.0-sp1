...
$(function () {
    ...
    var updateNotice = function () {
        IWB_API.updateNotice("updatenotice" ,function (identity ,response) {
            var data;
            if (identity === "updatenotice" && response && response.ret === 0) {
                data = response.data;
                if (data.home > 0) { // 首页更新数

                    if (!window.iwbTimelineNotice) { // 使用timelineNotice的页面显示更新
                        return;
                    }

                    $("#newmessagecount").text(data.home);
                    if (!$("#newmessage").is(":visible")) {
                        $("#newmessage").fadeIn(500);
                    }
                }
                if (data.fans > 0) { // 听众更新数
                    $("#newfans").find("em").text(data.fans);
                    if (!$("#newfans").is(":visible")) {
                        $("#newfans").show();
                    }
                }
                
                if (data.mentions > 0) { // 提到我的广播更新
                    $("#newmention").find("span").text(data.mentions);
                    if (!$("#newmention").is(":visible")) {
                        $("#newmention").show();
                    }
                }

                if (data.private > 0) { // 私信数更新
                    $("#newmail").find("span").text(data.private);
                    if (!$("#newmail").is(":visible")) {
                        $("#newmail").show();
                    }
                }
    ...
    setInterval(updateNotice, 15 * 1000);
    updateNotice();
    
    $("#newmessage").click(function () {
        var self = $(this);
        var num = parseInt($("#newmessagecount").text(),10);
        var loading = $("#newmessageloading");
        if (loading.is(":visible")) {
            return;
        }
        loading.show();
        IWB_API.timelineMore("newmessage" 
            ,{
                timelinetype: (window.iwbTimelineMoreType || 1),
                num:num
            }
            ,function (identity ,response) {
                if (identity === "newmessage" && response && response.ret === 0) {

                    if (window.iwbInstantTimeline) { // 启用了即时写时间线的页面
                        $("#tmain > .needRemove").remove();
                    }

                    self.fadeOut(500 ,function () {
                        var data = response.data.replace(/[\r\n\t]/g,"");
                        var result = $(data);
                        result.css("background","#fff4d8");
                        result.find(".tyinyong").css("background","#fff4d8");
                        $("#tmain").prepend(result);
                        setTimeout(function () {
                            try{
                                result.animate({ backgroundColor: "white" }, 1000);
                                result.find(".tyinyong").animate({ backgroundColor: "#f8f8f8" }, 1000);
                            } catch(err) { // IE9 会抛异常
                                result.css({ backgroundColor: "white" });
                                result.find(".tyinyong").css({ backgroundColor: "#f8f8f8" });
                            }
                        } ,500);
...
// 发私信/删私信
$(function () {
    var createMailBox = function (receiver) {
        var boxId; // 私信对话框id
        var boxWidth = 570;
        var boxHeight = 200;
        var boxLeft = ($("body").width() - boxWidth) / 2;
        ...
        boxId = IWB_DIALOG._init({
           ...
           ,left: boxLeft
           ,getDOM: function () {
               var mailform = ["<form class=\"privateform\">"
                              ,"<table width=\"560\" align=\"center\">"
                              ,"<tr><th width=\"100\" height=\"45\" align=\"center\">收信人</th><td align=\"left\"><input type=\"text\" class=\"text\" value=\"" + receiver + "\" name=\"mailReciever\" size=\"36\"/> <span class=\"gray\">请输入你的听众的微博帐号</span></td></tr>"
                              ,"<tr><th align=\"center\">内　容</th><td align=\"left\"><textarea name=\"mailContent\" type=\"text\"></textarea></td></tr>"
                              ,"<tr><th height=\"45\"></th><td align=\"right\"><label id=\"mailtips\" class=\"gray\">还能输入140字</label> <input type=\"button\" name=\"sendmail\" value=\"发送\" class=\"save\"/></td></tr>"
                              ,"</table>"
                              ,"</form>"].join("");

               var mailContentBox; // 内容区
               var sendBtn; // 发送按钮
               var tipBox; // 提示区
               var recieverBox; // 收信人

               mailform = $(mailform);
               mailContentBox = mailform.find("textarea[name=mailContent]");
               recieverBox = mailform.find("input[name=mailReciever]");
               sendBtn = mailform.find("input[name=sendmail]");
               tipBox = mailform.find("#mailtips");

               mailContentBox.keyup(function () {
               ...
               sendBtn.click(function () {
                   var reciever = recieverBox.val();
                   var content = mailContentBox.val();
                   var errormsg;
                   ...
                   // 发送前再次检查字数
                   mailContentBox.focus();
                   mailContentBox.trigger("keyup");
                   ...
                   // 发私信
                   IWB_API.sendMail("sendmail" ,reciever ,content ,function (identity ,response) {
                               ...
                               callback: function () {
                                   ...
                                   if (outbox.length > 0) {
                                       if (response.ret === 0) {
                                           ...
                                           // 私信计数增加
                                           if (outboxCount.length > 0) {
                                               outboxNum = parseInt(outboxCount.text(),10);
                                               outboxNum ++;
                                               outboxCount.text(outboxNum);
                                           }
                                       }
                                   }
                               }
        ...
        if (receiver) {
            $("textarea[name=mailContent]").focus();
        } else {
            $("input[name=mailReciever]").focus();
        }
    };
    ...
    $(".deletemail").live({
        click: function () {
            ...
            IWB_DIALOG.confirmbox({
                ...
                ok: function () {
                       var msgid = that.attr("data-msgid");
                       IWB_API.del(msgid ,msgid ,function (identity ,response) {
                           if (response.ret === 0 ) {
                               $("#"+msgid).fadeOut(500);
                           } else {
                               IWB_DIALOG.tipbox("warning","删除失败");
