...
<div class="itemtitle"><h3>认证用户管理</h3></div>
...
<div class="itemtitle">
    <div class="cuspages right">
        <form id="searchuserform" name="searchuserform" method="post" action="/admin/certification/search">
            <select name="localauth">
                <option value="1" <!--{if $conditions.localauth == 1}-->selected<!--{/if}-->>认证用户</option>
                <option value="0" <!--{if $conditions.localauth == 0}-->selected<!--{/if}-->>全部用户</option>
            </select>
            <select name="type">
...
<table class="tb tb2">
	<!--{if !$users}-->
    <tr>
        <th class="partition" colspan="10">没有本地化认证的用户</th>
    </tr>
    <tr>
    	<td colspan="10">修改搜索条件，列出全部用户信息，给未认证用户进行本地化认证。<strong><a href="/admin/certification/search/localauth/0">显示全部用户</a></strong></td>
    </tr>
	<!--{else}-->
	
    <tr>
        <th class="partition" colspan="4">共搜索到<strong> <!--{$userscount}--> </strong>名符合条件的用户</th>
        <th class="partition" align="right"  width="90"><a href="/admin/certification/search/localauth/0">显示全部用户</a></th>
    </tr>
    <tr class="header">
        <th width="20">&nbsp;</th>
        <th width="120">帐号</th>
    ...
    <tr>
        ...
        <td><!--{$user.nickname}--></td>
        <td><!--{$user.localauthtext}--></td>
        <td>
                <!--{if $user.localauth!=1}-->
            <a href="/admin/certification/auth/uid/<!--{$user.uid}-->/auth/1">认证</a>
                <!--{else}-->
            <a href="/admin/certification/auth/uid/<!--{$user.uid}-->/auth/0">取消认证</a>
                <!--{/if}-->
        </td>
    </tr>
        <!--{/foreach}-->
