...
<ul class="topiclist">
    <!--{foreach key=account item=item from=$component.contents}-->
        <li>
            <a title='<!--{$item.nick|escape:"html"}-->(@<!--{$item.name|escape:"html"}-->)' href="/u/<!--{$item.name|iurlencode}-->" class="fleft"><img title='<!--{$item.nick|escape:"html"}-->(@<!--{$item.name|escape:"html"}-->)' src="<!--{if $item.head}--><!--{$item.head}-->/100<!--{else}-->http://mat1.gtimg.com/www/mb/images/head_100.jpg<!--{/if}-->" /></a>
            <div class="fright">
            ...
            <p title='<!--{$item.introduction|escape:"html"}-->'><!--{$item.introduction|itruncate:60:"..."}--></p>
